<template>
  ...
  >
    ...
    <template #body-cell-tieuChi="{ row }">
      <q-td>
        <b-criterion :criterion="row.tieuChi" />
      </q-td>
    </template>
    <template #body-cell-soGio="{ value }">
...
import { ChiTietGioNCKHDanhDinhForm, GioNCKHDanhDinhForm } from './forms';
import { GioNCKHDanhDinhColumns } from './gio-nckh-danh-dinh.columns';

const title = 'Giờ nghiên cứu khoa học danh định';
const apiName = 'gio-nghien-cuu-khoa-hoc-danh-dinh';
...
const {
  ...
  name: apiName,
  title: title,
  columns: GioNCKHDanhDinhColumns,
  components: {
    addForm: GioNCKHDanhDinhForm,
    editForm: GioNCKHDanhDinhForm,
    viewForm: ChiTietGioNCKHDanhDinhForm,
  },
  events: {
    onAdd: (model) => {
      model.namApDung = DEFAULT_YEAR;
      return model;
    },
  },
