...
import { HeSoNangLucColumns } from './he-so-nang-luc.columns';
import { HeSoNangLucForm, ChiTietHeSoNangLucForm } from './forms';
import { DEFAULT_YEAR, YEARS } from 'src/constants';

const title = 'Hệ số năng lực';
const apiName = 'he-so-nang-luc';
...
const {
  ...
  name: apiName,
  title: title,
  columns: HeSoNangLucColumns,
  components: {
    addForm: HeSoNangLucForm,
    editForm: HeSoNangLucForm,
    viewForm: ChiTietHeSoNangLucForm,
  },
  events: {
    onAdd: (model) => {
      model.heSo = 0;
